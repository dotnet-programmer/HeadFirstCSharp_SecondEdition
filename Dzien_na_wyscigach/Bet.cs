namespace Dzien_na_wyscigach
{
	internal class Bet
	{
		...
		public string GetDescription() => Amount > 0 ? Bettor.Name + " obstawił " + Amount + " zł na psa numer " + Dog : Bettor.Name + " nie zawarł zakładu";

		// Parametrem jest zwyciezca wyscigu, jezeli pies wygral zwroc wartosc postawiona, jak nie zwroc ujemna wartosc postawiona
		public int PayOut(int Winner) => Dog == Winner ? Amount : -Amount;
	}
}